...
vim.opt.backup = true                  -- Создавать backup-файлы
vim.opt.backupdir = nvim_data .. '/backup//'  -- Централизованная директория для backup-файлов
vim.opt.directory = nvim_data .. '/swap//'    -- Централизованная директория для swap-файлов
vim.opt.undodir = nvim_data .. '/undo//'      -- Централизованная директория для undo-файлов
vim.opt.undofile = true               -- Сохранять историю отмен между сессиями

-- Создать директории, если они ещё не существуют
vim.fn.mkdir(vim.opt.backupdir:get()[1], 'p', true)
vim.fn.mkdir(vim.opt.directory:get()[1], 'p', true)
vim.fn.mkdir(vim.opt.undodir:get()[1], 'p', true)
